...
function PricingTabs({ activeTab, setActiveTab, className }: PricingTabsProps) {
  return (
    ...
    >
      <button
        className={cn(
          'relative z-[1] px-3 h-8 flex items-center justify-center cursor-pointer text-primary text-sm font-medium duration-200 shrink-0'
        )}
      >
        Pricing Plan
      </button>
...
function PricingTier({
      ...
      switch (response.status) {
        ...
        case 'scheduled':
          ...
          const downgradeMessage =
            response.details?.is_upgrade === false
  ...
  let buttonText = isAuthenticated ? 'Select Plan' : 'Upgrade Plan';
